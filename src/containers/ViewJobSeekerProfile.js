...
export default () => {
  ...
  React.useEffect(() => {
    axios
      .get(apiPath + "/jobseeker/edit-profile")
      .then((response) => {
  ...
  const handleClick = () => {
    axios
      .post(apiPath + "/request-to-activate-jobseeker-account")
      .then((response) => {
        if (response.data.resp === 1) {
      ...
      {loading ? (
        ...
        <>
          <div className="row my-30">
            ...
            <div className="col-lg-6">
              Last Name:{" "}
              <span className="text-primary">{profile?.last_name} </span>
            </div>
          </div>
          ...
          {isAccountDeactivated && (
            <div className="form-group my-30">
              <p className="text-danger">
                Your account is deactivated. You won't be able to apply for job.
              </p>
              <button
